...
export default function UsersPage() {
  ...
  useEffect(() => {
    const checkLoginStatus = async () => {
      ...
      await fetchUsers(); // Fetch users if token is available
    };

    const fetchUsers = async () => {
      try {
        const res = await fetch('http://backend-gamma-fawn.vercel.app/api/users');
        if (!res.ok) {
    ...
    // Check login status and fetch users on component mount
    checkLoginStatus();
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`http://backend-gamma-fawn.vercel.app/api/users/${id}`, {
        method: 'DELETE',
        headers: {
